...
public class RandomStateGenerator {
     ...
     * @return State Object.
     */
     protected State getConstrainedRandomState() {
        List<Integer> stateValues = new ArrayList<Integer>();
        Integer index = 0;
        List<Integer> boundaryValues = new ArrayList<Integer>();
        boundaryValues.addAll(Problem.boundaries);
        ...
        for (int j = 0; j < Problem.numberOfTasks; j++) {
            ...
            for (i = 0; i < Problem.numberOfResources; i++) {
                if ((boundaryValues.get(i) - Problem.weights.get(i).get(index)) >= 0) {
                    tempBoundryValues.add(boundaryValues.get(i) - Problem.weights.get(i).get(index));
                } else {
                    break;
                }
            }
            ...
            if (i == Problem.numberOfResources) {
                ...
                boundaryValues.addAll(tempBoundryValues);
